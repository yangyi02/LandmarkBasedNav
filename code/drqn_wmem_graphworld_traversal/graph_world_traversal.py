...
class GraphWorld(object):
    ...
    def prepare_seq(self):
        ...
        traverse_triplets = -np.ones([len(self.path[self.src_state])-1, 3])
        traverse_triplets[0,0] = self.src_state
        for i in range(traverse_triplets.shape[0]):
            traverse_triplets[i,2] = self.edgelabel[self.path[self.src_state][i]][self.path[self.src_state][i+1]]
        triplets = np.concatenate([pad_vec, triplets, pad_vec, traverse_triplets, pad_vec], axis=0).astype('int32')
        ...
        encoding[[0,des_len+1,-1], -2] = 1
        predflag = np.zeros([encoding.shape[0]])
        target_vec = np.zeros([encoding.shape[0],encoding.shape[1]-2])
        encoding_a = np.zeros([traverse_triplets.shape[0], encoding.shape[1]])
        encoding_a[:, -1] = 1
        predflag_a = np.ones(traverse_triplets.shape[0])
        target_vec_a = np.zeros([traverse_triplets.shape[0], target_vec.shape[1]])
        traverse_triplets_full = np.zeros([len(self.path[self.src_state])-1, 3])
        for i in range(traverse_triplets_full.shape[0]):
            traverse_triplets_full[i,0] = self.path[self.src_state][i]
            traverse_triplets_full[i,1] = self.path[self.src_state][i+1]
            traverse_triplets_full[i,2] = self.edgelabel[self.path[self.src_state][i]][self.path[self.src_state][i+1]]
        target_vec_a = GraphWorld.convert_triplets_to_encoding(traverse_triplets_full.astype('int32'), self.ndigits, self.nway)
