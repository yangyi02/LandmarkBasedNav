...
class DRQN_planner(object):
    ...
    def add_loss_op(self, pred_action, target_action, pred_flag):
        ...
        ndigits = self.config.ndigits
        nway = self.config.nway
        pred_action = tf.reshape(pred_action, shape=[batch_size, state_history, 2*(num_classes-1)*ndigits, nway])
        target_action = tf.reshape(target_action, shape=[batch_size, state_history, 2*(num_classes-1)*ndigits, nway])
    ...
    def build(self):
        ...
        with self.current_graph.as_default():
            ...
            ndigits = self.config.ndigits
            nway = self.config.nway
            self.q = tf.nn.softmax(tf.reshape(self.q_logits, shape=[batch_size, state_history, 2*(num_classes-1)*ndigits, nway]))
            self.q = tf.one_hot(tf.argmax(self.q, axis=3), depth=nway)
            self.q = tf.reshape(self.q, shape=(batch_size, state_history, 2*(num_classes-1)*ndigits*nway))
