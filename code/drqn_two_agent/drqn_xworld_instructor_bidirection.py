...
class DRQN_instructor(object):
    ...
    def get_best_action(self, state, h_state_fw, h_state_bw, slen, past_a):
        """
        Return best action
        ...
        """
        action_values, h_state_out_fw, h_state_out_bw = self.sess.run([self.q, self.hs_out_fw, self.hs_out_bw], feed_dict={self.s: state, self.hs_fw: h_state_fw, self.hs_bw: h_state_bw, self.slen: slen, self.past_a: past_a})
        return np.argmax(action_values[0]), action_values[0], h_state_out_fw, h_state_out_bw

    def get_action(self, state, h_state_fw, h_state_bw, slen, past_a):
        """
        ...
        Args:
            ...
            h_state: hidden state for rnn
        """
        best_action, q_values, h_state_out_fw, h_state_out_bw = self.get_best_action(state, h_state_fw, h_state_bw, slen, past_a)
    ...
    def policy(self):
        ...
        """
        return lambda state, h_state_fw, h_state_bw, slen, past_a: self.get_action(state, h_state_fw, h_state_bw, slen, past_a)
    ...
    def get_q_values_op(self, state, past_a, seq_len, h_state_fw, h_state_bw, scope, reuse=False):
        """
    ...
    def build(self):
        ...
        with self.current_graph.as_default():
            ...
            self.q, self.hs_out_fw, self.hs_out_bw = self.get_q_values_op(s, self.past_a, self.slen, self.hs_fw, self.hs_bw, scope="q", reuse=False)

            # compute Q values of next state
            sp = self.process_state(self.sp)
            self.target_q, self.hsp_out_fw, self.hsp_out_bw = self.get_q_values_op(sp, self.a, self.splen, self.hsp_fw, self.hsp_bw, scope="target_q", reuse=False)
    ...
    def train(self, model_a, exp_schedule, lr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            for i in range(200):
                ...
                best_action, q_values, h_state_fw, _ = self.get_best_action([q_input], h_state_fw, h_state_bw, slen, [action])
    ...
    def evaluate(self, model_a, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            for j in range(50):
                ...
                raw_goal_state, goal_state = self.convert_state_to_goal_state(state)
                action, action_q, h_state_fw, _ = self.get_action(raw_goal_state[None][None], h_state_fw, h_state_bw, slen, [action])
