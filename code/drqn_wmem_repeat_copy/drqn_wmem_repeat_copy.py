...
from utils.data_gen_repeat_copy import masked_sigmoid_cross_entropy, bitstring_readable, RepeatCopy
import tensorflow as tf
import tensorflow.contrib.layers as layers
from configs.drqn_wmem_repeat_copy import config
...
class DRQNwMemGraphWorld(object):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.float32, shape=(None, None, self.config.num_bits+2))
        ...
        self.target_s = tf.placeholder(tf.float32, shape=(None, None, self.config.num_bits+1)) # (nb*state_history, vec_len)
    ...
    def add_loss_op(self, pred_s, target_s, pred_flag):
        ...
        """
        self.loss = masked_sigmoid_cross_entropy(pred_s, target_s, pred_flag)
    ...
    def build(self):
        ...
        self.q = tf.nn.sigmoid(self.q_logits)
    ...
    def update_step(self, t, lr, batch_data):
        ...
        fd = {
            ...
            self.target_s: batch_data.target,
    ...
    def train(self, lr_schedule):
        ...
        rcopy = RepeatCopy(num_bits=self.config.num_bits, batch_size=self.config.batch_size,
            min_length=self.config.min_length, max_length=self.config.max_length,
            min_repeats=self.config.min_repeats, max_repeats=self.config.max_repeats)
    ...
    def evaluate(self, num_episodes=None):
        ...
        for i in range(num_episodes):
            config = self.config
            batch_data = rcopy()
            pred_state, h_state_out = self.sess.run([self.q, self.hs_out], feed_dict={self.s: batch_data.observations,
                self.slen: batch_data.seqlen,
                self.hs: DNC.zero_state(self.config, self.config.batch_size)})
            logits_mask = np.tile(np.expand_dims(batch_data.mask, 2), (1,1,self.config.num_bits+1))
            accs.append(np.sum(np.array((pred_state>0.5)==batch_data.target)*logits_mask)/np.sum(logits_mask))
