...
class XWorldItem(object):
    ...
    """
    def __init__(self, item_type='', index=-1, name='', class_name='', class_id=-1, location=numpy.empty([0]), orientation=numpy.empty([0]), image_name='', image=numpy.empty([0])):
        """
    ...
    def update_orientation(self, action):
        if action=='turn_right': # image x-y coordinate system has y flip
            self.orientation = numpy.dot(numpy.array([[0,-1],[1,0]]), self.orientation)
    ...
    def get_next_location(self, velocity):
        ...
        Velocity indicates moving forward or backward (+/-1)
        ...
        """
        if self.is_movable():
            next_location = self.location + velocity * self.orientation
        else:
    ...
    def is_movable(self, next_item_type='empty'):
        ...
        #    return True
            ...
            return True
        #elif self.item_type == 'agent' and next_item_type == 'river':
        #    return True
    ...
    def to_be_removed(self, next_item_type='empty'):
        ...
        #    return True
        if self.item_type == 'box' and next_item_type == 'river':
            return True
