...
class Generator():
    def __init__(self, width=64, height=64, max_rooms=15, min_room_xy=5,
                 max_room_xy=10, rnd_seed=None, rooms_overlap=False, random_connections=1,
    ...
    def gen_level(self):
        ...
        # insert object
        for x in self.corridor_list:
            for y in x:
                if self.level[y[1]][y[0]-1]=='wall' and self.level[y[1]][y[0]+1]=='wall':
                    if random.randint(0,1):
                        self.level[y[1]][y[0]-1] = 'object'
                    else:
                        self.level[y[1]][y[0]+1] = 'object'
                if self.level[y[1]-1][y[0]]=='wall' and self.level[y[1]+1][y[0]]=='wall':
                    if random.randint(0,1):
                        self.level[y[1]-1][y[0]] = 'object'
                    else:
                        self.level[y[1]+1][y[0]] = 'object'
    ...
    def gen_tiles_level(self):
        ...
        for row_num, row in enumerate(self.level):
            ...
            for col_num, col in enumerate(row):
                ...
                if col == 'object':
                    tmp_tiles.append(self.tiles['object'])

            self.tiles_level.append(''.join(tmp_tiles))        

 
if __name__ == '__main__':
    gen = Generator(width=18, height=18, max_rooms=5, min_room_xy=4,
                 max_room_xy=6, rnd_seed=None, rooms_overlap=False, random_connections=1,
                 random_spurs=3, tiles=CHARACTER_TILES)
    gen.gen_level()
    gen.gen_tiles_level()
