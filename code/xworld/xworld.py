...
class XWorld(object):
    ...
    def reset(self):
        ...
        self.num_step = 0
        return self.state.onehot_state
        '''
        locidx = self.state.agent_location[1]*3+self.state.agent_location[0]
        oriidx = (self.state.agent_orientation[1]+1)*2+self.state.agent_orientation[0]
        stateidx = (locidx+1)*(oriidx+1)-1
        statevec = numpy.zeros(45)
        statevec[stateidx] = 1
        return statevec
        '''
    ...
    def step(self, action):
        ...
        self.num_step += 1
        return self.state.onehot_state, self.teacher.reward, self.teacher.done
        '''
        locidx = self.state.agent_location[1]*3+self.state.agent_location[0]
        oriidx = (self.state.agent_orientation[1]+1)*2+self.state.agent_orientation[0]
        stateidx = (locidx+1)*(oriidx+1)-1
        statevec = numpy.zeros(45)
        statevec[stateidx] = 1
        return statevec, self.teacher.reward, self.teacher.done
        '''
    ...
    def display(self, win_i=1):
        ...
        plt.subplot(2, 3, 3)
        plt.imshow(self.teacher.goal_obs_image)
        plt.xticks([]), plt.yticks([])  # to hide tick values on X and Y axis
