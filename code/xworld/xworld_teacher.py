...
class XWorldTeacher(object):
    ...
    def update_out_border_reward(self, agent, state, action, next_state, num_step):
        ...
        agent_orientation = state.xmap.items[agent_id].orientation
        width, height = state.xmap.dim['width'], state.xmap.dim['height']
        if agent_location[0] == 0 and agent_orientation[0] * agent.velocity[action] == -1:
            self.rewards['out_border'] = -0.1
        elif agent_location[1] == 0 and agent_orientation[1] * agent.velocity[action] == -1:
            self.rewards['out_border'] = -0.1
        elif agent_location[0] == width-1 and agent_orientation[0] * agent.velocity[action] == 1:
            self.rewards['out_border'] = -0.1
        elif agent_location[1] == height-1 and agent_orientation[1] * agent.velocity[action] == 1:
            self.rewards['out_border'] = -0.1
