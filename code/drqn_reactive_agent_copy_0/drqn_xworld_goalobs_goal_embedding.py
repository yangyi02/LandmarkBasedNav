...
from xworld import xworld_navi_goal_obs, xworld_args
...
from configs.drqn_xworld import config
...
class DRQN(object):
    ...
    def add_placeholders_op(self):
        ...
        state_shape = list([self.env.args.visible_radius_unit_front+1, 2*self.env.args.visible_radius_unit_side+1, len(self.env.state.xmap.item_class_id)+1])
        self.s = tf.placeholder(tf.bool, shape=(None, None, state_shape[0], state_shape[1], state_shape[2]))
        self.gs = tf.placeholder(tf.bool, shape=(None, 1, state_shape[0], state_shape[1], state_shape[2]))
    ...
    def get_q_values_op(self, state, goal_state, past_a, seq_len, h_state, scope, reuse=False):
        ...
        goal_state = tf.reshape(goal_state, shape=[-1, 1, 1, state_shape[0]*state_shape[1]*state_shape[2]])
        with tf.variable_scope(scope, reuse = False):
            state = layers.fully_connected(state, 200, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer(), scope='siamese_l1')
            state = layers.fully_connected(state, 100, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer(), scope='siamese_l2')
        with tf.variable_scope(scope, reuse = True):
            goal_state = layers.fully_connected(goal_state, 200, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer(), scope='siamese_l1')
            goal_state = layers.fully_connected(goal_state, 100, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer(), scope='siamese_l2')
        with tf.variable_scope(scope, reuse = reuse):
            lstm_cell = tf.nn.rnn_cell.LSTMCell(num_units=h_size)
            out = tf.concat([state, past_a], axis=2)
            ...
            goal_state = layers.fully_connected(goal_state, int(h_size/num_actions), activation_fn = None, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer())
            out = tf.reshape(out, shape=[-1, max_seq_len, num_actions, int(h_size/num_actions)])
            out = tf.reduce_sum(tf.multiply(out, goal_state), axis=3)
            out = tf.reshape(out, shape=[-1, num_actions])
...
if __name__ == '__main__':
    ...
    env = xworld_navi_goal_obs.XWorldNaviGoal(args)
