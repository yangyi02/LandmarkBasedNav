...
class DRQN(object):
    ...
    def add_placeholders_op(self):
        ...
        #self.gs = tf.placeholder(tf.bool, shape=(None, 1, state_shape[2]))
        self.gs = tf.placeholder(tf.bool, shape=(None, None, state_shape[0], state_shape[1]))
    ...
    def get_q_values_op(self, state, goal_state, past_a, seq_len, h_state, scope, reuse=False):
        ...
        goal_state = tf.reshape(goal_state, shape=[-1, max_seq_len, state_shape[0]*state_shape[1]])
        #goal_state = tf.reshape(goal_state, shape=[-1, 1, state_shape[2]])
        #goal_state = tf.tile(goal_state, multiples=[1, max_seq_len, 1])
        with tf.variable_scope(scope, reuse = reuse):
            state = layers.fully_connected(state, 200, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer())
            state = layers.fully_connected(state, 100, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer())
            goal_state = layers.fully_connected(goal_state, 100, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer())
            goal_state = layers.fully_connected(goal_state, 50, activation_fn = tf.nn.relu, weights_initializer=layers.xavier_initializer(), biases_initializer=tf.zeros_initializer())
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        curr_attention_batch = np.equal(np.sum(np.equal(s_batch, np.reshape(gs_batch, [gs_batch.shape[0], 1, 1, 1, gs_batch.shape[2]])), 4), s_batch.shape[4])
    ...
    def train(self, exp_schedule, lr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            for i in range(200):
                ...
                curr_attention = np.equal(np.sum(np.equal(q_input,goal_state[None][None][None]),3), q_input.shape[3])
                best_action, q_values, h_state = self.get_best_action([q_input], curr_attention[None], h_state, slen, [action])
    ...
    def evaluate(self, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            for j in range(50):
                ...
                curr_attention = np.equal(np.sum(np.equal(q_input,goal_state[None][None][None]),3), q_input.shape[3])
                action, action_q, h_state = self.get_action([q_input], curr_attention[None], h_state, slen, [action])
