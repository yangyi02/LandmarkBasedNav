...
class DRQN(object):
    ...
    def get_q_values_op(self, state, goal_state, past_a, seq_len, h_state, scope, reuse=False):
        ...
        state_shape = list([self.env.args.visible_radius_unit_front+1, 2*self.env.args.visible_radius_unit_side+1, len(self.env.state.xmap.item_class_id)+1])
        state = tf.reshape(state, shape=[-1, state_shape[0], state_shape[1], state_shape[2]])
        goal_state = tf.reshape(goal_state, shape=[-1, state_shape[0], state_shape[1], state_shape[2]])
        with tf.variable_scope(scope, reuse = False):
            state = layers.conv2d(state, num_outputs = 100, kernel_size = 3, scope='siamese_l1')
            state = layers.conv2d(state, num_outputs = 200, kernel_size = 3, scope='siamese_l2')
            state = layers.max_pool2d(state, kernel_size = [3, 3], stride = 3, scope='siamese_l3')
        with tf.variable_scope(scope, reuse = True):
            goal_state = layers.conv2d(goal_state, num_outputs = 100, kernel_size = 3, scope='siamese_l1')
            goal_state = layers.conv2d(goal_state, num_outputs = 200, kernel_size = 3, scope='siamese_l2')
            goal_state = layers.max_pool2d(goal_state, kernel_size = [3, 3], stride = 3, scope='siamese_l3')
        with tf.variable_scope(scope, reuse = reuse):
            state = tf.reshape(state, shape=[-1, max_seq_len, 1*2*200])
            goal_state = tf.reshape(goal_state, shape=[-1, 1, 1*2*200])
            ...
            out = layers.fully_connected(out, 200)
