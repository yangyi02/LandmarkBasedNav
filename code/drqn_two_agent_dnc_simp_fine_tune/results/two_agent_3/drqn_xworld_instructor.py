...
class DRQN_instructor(object):
    ...
    def evaluate2(self, model_a, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            num_ldms.append(len(goal_state_batch))
            reward_a = 0
    ...
    def gen_sample_seq(self):
        ...
        for i in range(5):
            ...
            path_loc = self.env.teacher.compute_shortest_path_single_src(self.env.state, src_loc, tgt_loc)
            h_state = (np.zeros([1,self.config.h_size]),np.zeros([1,self.config.h_size]))
            action = 0
            slen = np.ones(1).astype('int32')
            encoding.append(list())
            for j, loc in enumerate(path_loc):
                state = full_onehot_state[loc[1]:loc[1]+3, loc[0]:loc[0]+3, :]
