...
def weighted_softmax(activations, strengths, strengths_op):
  ...
  sharp_activations = activations * (1+transformed_strengths)
  normalized_activations = tf.nn.softmax(sharp_activations)
...
def batch_invert_permutation(permutations):
  ...
  with tf.name_scope('batch_invert_permutation', values=[permutations]):
    inverses = tf.map_fn(tf.invert_permutation, permutations)
    return inverses
...
def batch_gather(values, indices):
  ...
  with tf.name_scope('batch_gather', values=[values, indices]):
    result = tf.map_fn(lambda x: tf.gather(x[0], x[1]), (values, indices), dtype=tf.float32)
    return result
...
class MaskedRotInvariantCosineWeights(object):
  """Cosine-weighted attention.
  Calculates the cosine similarity between a query and each word in memory, then
  applies a weighted softmax to return a sharp distribution.
  """

  def __init__(self, mask_size=9,
               strength_op=tf.nn.softplus):
    """Initializes the CosineWeights module.
    Args:
      num_heads: number of memory heads.
  ...
  def __call__(self, memory, mask, keys, strengths):
    """Connects the CosineWeights module into the graph.
    Args:
      memory: A 3-D tensor of shape `[batch_size, memory_size, word_size]`.
      keys: A 4-D tensor of shape `[batch_size, num_heads, word_size, 4]`, 4 denotes for orientations, assume num_heads=1 here
      mask: A 3-D tensor of shape `[batch_size, memory_size, mask_size]`
      strengths: A 2-D tensor of shape `[batch_size, num_heads]`.
    Returns:
      Weights tensor of shape `[batch_size, num_heads, memory_size]`.
    """
    sqr_len = np.sqrt(self._mask_size).astype('int32')
    batch_size = tf.shape(memory)[0]
    memory_size = tf.shape(memory)[1]
    word_size = tf.shape(memory)[2]
    memory = tf.reshape(memory, shape=[batch_size, memory_size, sqr_len, sqr_len, -1])
    mask = tf.reshape(mask, shape=[batch_size, memory_size, sqr_len, sqr_len, 1])
    memory = tf.reshape(memory*mask, shape=[batch_size, memory_size, -1])
    memory_norms = _vector_norms(memory)
    similarity = -tf.ones([batch_size, 1, memory_size])
    for i in range(4):
      cur_keys = keys[:,:,:,i]
      cur_keys = tf.reshape(cur_keys, shape=[batch_size, 1, sqr_len, sqr_len, -1])
      cur_keys = cur_keys*mask
      cur_keys = tf.reshape(cur_keys, shape=[batch_size, memory_size, -1])

      dot = tf.reduce_sum(cur_keys*memory, 2)
      key_norms = _vector_norms(cur_keys)
      norm = tf.squeeze(memory_norms*key_norms, 2)
      similarity = tf.maximum(tf.expand_dims(dot/(norm+_EPSILON),1), similarity)

    return weighted_softmax(similarity, strengths, self._strength_op)

  def get_sim(self, memory, mask, keys):
    """Connects the CosineWeights module into the graph.
    Args:
      memory: A 3-D tensor of shape `[batch_size, memory_size, word_size]`.
      keys: A 4-D tensor of shape `[batch_size, num_heads, word_size, 4]`, 4 denotes for orientations, assume num_heads=1 here
      mask: A 3-D tensor of shape `[batch_size, memory_size, mask_size]`
    Returns:
      similarity tensor of shape `[batch_size, num_heads, memory_size]`.
    """
    sqr_len = np.sqrt(self._mask_size).astype('int32')
    batch_size = tf.shape(memory)[0]
    memory_size = tf.shape(memory)[1]
...
class TemporalLinkage(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, write_weights, prev_state, flag):
  ...
  def _precedence_weights(self, prev_precedence_weights, write_weights, flag):
    ...
    Args:
      ...
      flag: A tensor of shape `[batch_size]`
    Returns:
    ...
    with tf.name_scope('precedence_weights'):
      ...
      return ((1 - write_sum) * prev_precedence_weights + write_weights)*(1-tf.expand_dims(tf.expand_dims(flag,1),2))

  @staticmethod
  def zero_state(config, batch_size, dtype=np.float32):
    return TemporalLinkageState(link=np.zeros([batch_size, config.num_writes, config.memory_size, config.memory_size], dtype=dtype),\
          precedence_weights=np.zeros([batch_size, config.num_writes, config.memory_size], dtype=dtype))

  @staticmethod
  def state_placeholder(config, dtype=tf.float32):
    return TemporalLinkageState(link=tf.placeholder(dtype, shape=(None, config.num_writes, config.memory_size, config.memory_size)),\
          precedence_weights=tf.placeholder(dtype, shape=(None, config.num_writes, config.memory_size)))
...
class Freeness(tf.nn.rnn_cell.RNNCell):
  ...
  *   Decreased by reading, where usage is decreased after reading from a
  ...
  def __call__(self, write_weights, prev_usage):
    ...
    write_weights = tf.stop_gradient(write_weights) # eric, is it true?
  ...
  def _usage_after_read(self, prev_usage, free_gate, read_weights):
    ...
    with tf.name_scope('usage_after_read'):
      ...
      phi = tf.reduce_prod(1 - free_read_weights, axis=1, name='phi')
...
if __name__ == '__main__':
    mask_size = 4
    sqr_len = np.sqrt(mask_size).astype('int32')
    sim = MaskedRotInvariantCosineWeights(mask_size=mask_size)
    ndigits = 1
    nway = 2
    memory = tf.placeholder(tf.float32, shape=(None, 3, mask_size*ndigits*nway))
    mask = tf.placeholder(tf.float32, shape=(None, 3, mask_size))
    keys = tf.placeholder(tf.float32, shape=(None, 1, mask_size*ndigits*nway, 4))
    strengths = tf.placeholder(tf.float32, shape=(None, 1))
    mrsim = sim(memory, mask, keys, strengths)

    batch_size = 2
    sess = tf.Session()
    keys_uor = (np.random.random([batch_size, sqr_len, sqr_len, ndigits*nway, 1])>0.5).astype('float32')
    keys_allor = list()
    keys_allor.append(keys_uor)
    for i in range(1,4):
      keys_allor.append(np.rot90(keys_uor, k=i, axes=(1,2)))

    fd = {
      memory: (np.random.random([batch_size, 3, mask_size*ndigits*nway])>0.5).astype('float32'),
      mask: (np.random.random([batch_size, 3, mask_size])>0.5).astype('float32'),
      keys: np.concatenate(keys_allor, axis=4).reshape([batch_size, 1, mask_size*ndigits*nway, 4]),
      strengths: 1*np.ones([batch_size,1])
    }
    init = tf.global_variables_initializer()
    sess.run(init)
    mrsim_eva = sess.run([mrsim], feed_dict=fd)
    print(fd, mrsim_eva)