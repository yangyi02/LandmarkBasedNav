...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        agate_batch = list()
        next_agate_batch = list()
        for idx in idxes:
            obs, act, agate, rew, dmsk, smsk, seqlen = self._encode_observation2(idx)
            ...
            agate_batch.append(agate)
            next_obs, _, next_agate, _, _, _, next_seqlen = self._encode_observation2(idx+1)
            next_obs_batch.append(next_obs[None])
            next_agate_batch.append(next_agate)
        ...
        agate_batch = np.concatenate(agate_batch, 0)
        next_agate_batch = np.concatenate(next_agate_batch, 0)
        ...
        return obs_batch, seqlen_batch, act_batch, past_act_batch, agate_batch, next_agate_batch, rew_batch, done_mask, seq_mask, next_obs_batch, next_seqlen_batch
    ...
    def _encode_observation2(self, idx):
        ...
        if missing_context > 0:
            ...
            for _ in range(missing_context):
                ...
                dmsk.append(1.0)
                smsk.append(0.0)
            return np.concatenate(frames, 0), np.array(act), np.array(agate), np.array(rew), np.array(dmsk), np.array(smsk), end_idx-start_idx
        else:
            # this optimization has potential to saves about 30% compute time \o/
            img_h, img_w, img_c = self.obs.shape[1], self.obs.shape[2], self.obs.shape[3]
            return self.obs[start_idx:end_idx].reshape(end_idx-start_idx,img_h,img_w,img_c), self.action[start_idx:end_idx],\
             self.agate[start_idx:end_idx], self.reward[start_idx:end_idx], np.array([1.0 if x else 0.0 for x in self.done[start_idx:end_idx]]),\
             np.array([0.0]*np.floor((end_idx-start_idx)/2).astype('int32')+[1.0]*np.ceil((end_idx-start_idx)/2).astype('int32')+[0.0]*(self.frame_history_len-end_idx+start_idx)), end_idx-start_idx
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.agate    = np.empty([self.size],                     dtype=np.float32)
    ...
    def store_effect(self, idx, action, agate, reward, done):
