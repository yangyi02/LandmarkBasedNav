...
from utils.mem_graph_world import MemoryAccess
...
class DNC(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, prev_state):
    ...
    Args:
      inputs: Tensor input of shape (batch_size, state_size+2 <one reading flag and one writing flag>)
    ...
    controller_input = tf.concat(
        [inputs[:,:-2], tf.reshape(prev_access_output, shape=[-1, self._config.num_reads*self._config.word_size])], 1)
    ...
    access_output, access_state = self._access(tf.concat([controller_output, inputs[:,-2:]], axis=1), prev_access_state)
...
if __name__ == '__main__':
    ...
    inputs = tf.placeholder(tf.float32, shape=(None, 6))
    ...
    fd = {
      ...
      inputs: np.random.random([batch_size, 6]),
    }
