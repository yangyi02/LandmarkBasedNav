...
from configs.drqn_wmem_xworld import config

class DRQNwMem(object):
    """
    ...
    def policy(self):
        ...
        """
        return lambda state, h_state, slen: self.get_action(state, h_state, slen)
    ...
    def get_q_values_op(self, state, past_a, seq_len, h_state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            out = tf.reshape(out, shape=[-1, state_shape[0]*state_shape[1]*state_shape[2]])
            out = tf.concat([out, past_a], axis=1)
            out = tf.reshape(out, shape=[-1, state_shape[0]*state_shape[1]*state_shape[2]+num_actions])
    ...
    def evaluate(self, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            for j in range(50):
                ...
                action, action_q, h_state = self.get_action([q_input], h_state, slen, [action])
