...
class DRQN(object):
    ...
    def evaluate(self, env=None, num_episodes=None):
        ...
        ldm_map = np.zeros([height, width])
        ldm_map_norm = np.zeros([height, width])
        for i in range(num_episodes):
            ...
            slen = np.ones(1).astype('int32')
            action = 0
            ldm_map_norm[env.teacher.goal_location[1], env.teacher.goal_location[0]] += 1
            for j in range(50):
                ...
                if done:
                    break
            ldm_map[env.teacher.goal_location[1], env.teacher.goal_location[0]] += env.teacher.goal_finish
            # updates to perform at the end of an episode
            rewards.append(total_reward)   

        plt.imshow(ldm_map/(ldm_map_norm+1e-6), cmap='hot', interpolation='nearest')
        plt.show()
        plt.pause(100)  
