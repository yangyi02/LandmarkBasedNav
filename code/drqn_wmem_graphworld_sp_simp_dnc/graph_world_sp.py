...
class GraphWorld(object):
    ...
    def prepare_seq_simp(self):
        idx = np.concatenate(self.graph.edges())
        nmax = pow(self.nway, self.ndigits)
        rnddict = np.random.permutation(nmax)
        idx = np.array([rnddict[i] for i in idx])
        encoding = GraphWorld.convert_idx_to_encoding(idx.astype('int32'), self.ndigits, self.nway)
        encoding = np.concatenate([encoding, np.zeros([encoding.shape[0],1])], 1)
        encoding[1::2,-1] = 1
        path_idx = np.array([rnddict[i] for i in self.path[self.src_state]])
        target = GraphWorld.convert_idx_to_encoding(path_idx.astype('int32'), self.ndigits, self.nway)
        graph_permed = np.zeros([pow(self.nway, self.ndigits),pow(self.nway, self.ndigits)])
        for i in range(self.n_node):
            for idx, j in enumerate(self.graph.neighbors(i)):
                    graph_permed[rnddict[i]][rnddict[j]] = 1
                    graph_permed[rnddict[j]][rnddict[i]] = 1
        return encoding, target, graph_permed
    ...
    @staticmethod
    def convert_idx_to_encoding(idx, ndigits, nway):
        encoding = np.zeros([idx.shape[0], nway*ndigits])
        for i in range(ndigits):
            encoding[np.arange(idx.shape[0]), idx%nway+i*nway] = 1
            idx = np.floor(idx/nway).astype('int32')
        return encoding
    ...
    @staticmethod
    def convert_encoding_to_idx(encoding, ndigits, nway):
        idx = np.zeros(encoding.shape[0])
        for i in range(ndigits):
            idx = idx+pow(nway,i)*\
                    np.argmax(encoding[:,i*nway:(i+1)*nway], axis=1)
...
if __name__ == '__main__':
    ...
    encoding, target_action = gworld.prepare_seq_simp()
    print(encoding, target_action)
    print(GraphWorld.convert_encoding_to_idx(encoding[:,:-1], 2, 2))
