...
from utils.dnc_simp import DNC
...
DatasetTensors = collections.namedtuple('DatasetTensors', ('observations','target', 'mask','seqlen','src_inputs','tgt_inputs'))
...
class DRQNwMemGraphWorld(object):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.float32, shape=(None, None, self.config.ndigits*self.config.nway+1))
        self.hs = DNC.state_placeholder(self.config)
        self.slen = tf.placeholder(tf.int32, shape=(None))
        self.src_inputs = tf.placeholder(tf.float32, shape=(None, self.config.ndigits*self.config.nway))
        self.tgt_inputs = tf.placeholder(tf.float32, shape=(None, self.config.ndigits*self.config.nway))
        self.pred_flag = tf.placeholder(tf.float32, shape=(None, None)) # (nb, state_history)
        self.target_action = tf.placeholder(tf.float32, shape=(None, None, self.config.ndigits*self.config.nway)) # (nb, state_history, num_actions)
        self.lr = tf.placeholder(tf.float32, shape=(None))

    def get_q_values_op(self, state, seq_len, h_state, target_action, src_inputs, tgt_inputs, scope, reuse=False):
        """
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            out = dnc_cell.value_iteration(h_state_out.access_state, src_inputs, tgt_inputs, tf.shape(target_action)[1])
            #### out has size (nb, seq_len, self.config.ndigits*self.config.nway) #####
            batch_size = tf.shape(out)[0]
            state_history = tf.shape(out)[1]
            out = tf.reshape(out, shape=[batch_size, state_history, self.config.ndigits, self.config.nway])
            out = out/(tf.reduce_sum(out, axis=3, keep_dims=True)+1e-6)
            out = tf.log(out+1e-6)
            out = tf.reshape(out, shape=[batch_size, state_history, -1])
            Q = dnc_cell.Q
        return out, h_state_out, Q
    ...
    def add_loss_op(self, pred_action, target_action, pred_flag):
        ...
        pred_action = tf.reshape(pred_action, shape=[batch_size, state_history, self.config.ndigits, self.config.nway])
        target_action = tf.reshape(target_action, shape=[batch_size, state_history, self.config.ndigits, self.config.nway])
    ...
    def build(self):
        ...
        self.q_logits, self.hs_out, self.Q = self.get_q_values_op(s, self.slen, self.hs, self.target_action, self.src_inputs, self.tgt_inputs, scope="q", reuse=False)
        batch_size = tf.shape(self.q_logits)[0]
        state_history = tf.shape(self.q_logits)[1]
        encoding_len = tf.shape(self.q_logits)[2]
        self.q = tf.nn.softmax(tf.reshape(self.q_logits, shape=[batch_size, state_history, self.config.ndigits, self.config.nway]))
        self.q = tf.one_hot(tf.argmax(self.q, axis=3), depth=self.config.nway)
        self.q = tf.reshape(self.q, shape=(batch_size, state_history, encoding_len))
    ...
    def update_step(self, t, lr, batch_data):
        ...
        fd = {
            ...
            self.src_inputs: batch_data.src_inputs,
            self.tgt_inputs: batch_data.tgt_inputs,
    ...
    def train(self, beta_schedule, lr_schedule, cr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            for i in range(self.config.batch_size):
                ...
                encoding, target_action, _ = self.env.prepare_seq_simp()
                predflag = np.ones(target_action.shape[0])
                ...
                src_inputs_batch.append(target_action[[0]])
                tgt_inputs_batch.append(target_action[[-1]])
                if target_action.shape[0]>max_len:
                    max_len = target_action.shape[0]
                if encoding.shape[0]>max_encoding_len:
                    max_encoding_len = encoding.shape[0]

            batch_data = DatasetTensors(np.concatenate([np.concatenate([x, np.zeros([1, max_encoding_len-x.shape[1], x.shape[2]])], axis=1) for x in encoding_batch], axis=0),
                ...
                np.concatenate(src_inputs_batch, axis=0), np.concatenate(tgt_inputs_batch, axis=0))
    ...
    def evaluate(self, cr_schedule, curri_idx=None, env=None, num_episodes=None):
        ...
        for i in range(num_episodes):
            ...
            for j in range(self.config.batch_size):
                ...
                config.n_node, config.k_ring, config.p_rewiring, config.path_len_limit, config.planning_len = cr_schedule[curri_idx]
                self.env.reset(config) # h x w x c
                encoding, target_action, graph_permed = self.env.prepare_seq_simp()
                predflag = np.ones(target_action.shape[0])
                encoding_batch.append(encoding[None])
                predflag_batch.append(predflag[None])
                target_action_batch.append(target_action[None])
                ...
                src_inputs_batch.append(target_action[[0]])
                tgt_inputs_batch.append(target_action[[-1]])
                graph_permed_batch.append(graph_permed)
                if target_action.shape[0]>max_len:
                    max_len = target_action.shape[0]
                if encoding.shape[0]>max_encoding_len:
                    max_encoding_len = encoding.shape[0]
            ...
            pred_action, h_state = self.sess.run([self.q, self.hs_out], feed_dict={self.s: batch_data.observations, self.hs: h_state, self.slen: batch_data.seqlen, self.target_action:batch_data.target, self.src_inputs: batch_data.src_inputs, self.tgt_inputs: batch_data.tgt_inputs})
            #pred_action, h_state, Q = self.sess.run([self.q_logits, self.hs_out, self.Q], feed_dict={self.s: batch_data.observations, self.hs: h_state, self.slen: batch_data.seqlen, self.target_action:batch_data.target, self.src_inputs: batch_data.src_inputs, self.tgt_inputs: batch_data.tgt_inputs})
            for j in range(self.config.batch_size):
                graph_permed = graph_permed_batch[j]
                path_len = np.sum(batch_data.mask[j]).astype('int32')
                pred_path = GraphWorld.convert_encoding_to_idx(pred_action[j][:path_len,:], self.config.ndigits, self.config.nway)
                target_path = GraphWorld.convert_encoding_to_idx(batch_data.target[j][:path_len,:], self.config.ndigits, self.config.nway)
                acc = 1
                if pred_path[0]!=target_path[0] or pred_path[-1]!=target_path[-1]:
                    acc = 0
                for k in range(path_len-1):
                    if graph_permed[pred_path[k],pred_path[k+1]]==0:
                        acc = 0
                accs.append(acc)
...
if __name__ == '__main__':
    ...
    if config.deploy_only:
        ...
        shutil.copyfile('./configs/drqn_wmem_graph_world_sp.py', config.output_path+'config.py')
