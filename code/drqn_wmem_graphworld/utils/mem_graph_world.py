...
class MemoryAccess(tf.nn.rnn_cell.RNNCell):
  ...
  *   `addressing.FreenessAllocator` for keeping track of memory usage, where
  ...
  def __call__(self, inputs, prev_state):
    ...
    Args:
      inputs: tensor of shape `[batch_size, input_size+2 <one reading flag and one writing flag>]`. This is used to
    ...
    """
    read_flag = tf.expand_dims(tf.expand_dims(inputs[:,-2],1),2)
    write_flag = tf.expand_dims(tf.expand_dims(inputs[:,-1],1),2)
    inputs = self._read_inputs(inputs[:,:-2])
    ...
    write_weights = tf.multiply(write_weights, write_flag)
    ...
    read_weights = tf.multiply(read_weights, read_flag)
...
if __name__ == '__main__':
    ...
    inputs = tf.placeholder(tf.float32, shape=(None, 4, 5))
    ...
    fd = {
      ...
      inputs: np.concatenate([np.random.random([batch_size, 4, 3]), np.zeros([batch_size, 4, 1]), np.random.random([batch_size, 4, 1])], axis=2),
    }
