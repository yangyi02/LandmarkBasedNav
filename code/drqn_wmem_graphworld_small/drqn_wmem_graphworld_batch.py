...
class DRQNwMemGraphWorld(object):
    ...
    def add_placeholders_op(self):
        ...
        self.target_action = tf.placeholder(tf.float32, shape=(None, None, self.config.num_actions)) # (nb, state_history, num_actions)
    ...
    def get_q_values_op(self, state, seq_len, h_state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            #### out has size (nb, seq_len, num_actions) #####
    ...
    def add_loss_op(self, pred_action, target_action, pred_flag):
        ...
        """
        smx = tf.nn.softmax_cross_entropy_with_logits(labels=target_action, logits=pred_action)
    ...
    def train(self, beta_schedule, lr_schedule, cr_schedule):
        ...
        while t < self.config.nsteps_train:
            ...
            for i in range(self.config.batch_size):
                ...
                encoding, target_action, predflag = self.env.prepare_seq3()
        ...
        for i in range(num_episodes):
            ...
            for i in range(config.max_step_len):
                ...
                if self.config.use_transition_only_during_answering:
                    ...
                    current_encoding = GraphWorld.convert_triplets_to_encoding(np.array([[env.current_state, next_state, -1]]).astype('int32'), config.ndigits, config.nway)
