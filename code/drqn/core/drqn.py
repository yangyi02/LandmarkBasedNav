...
class DRQN(QN):
    ...
    def build(self):
        ...
        self.q, self.hs = self.get_q_values_op(s, self.hs, scope="q", reuse=False)

        # compute Q values of next state
        sp = self.process_state(self.sp)
        self.target_q, self.hsp = self.get_q_values_op(sp, self.hsp, scope="target_q", reuse=False)
    ...
    def get_best_action(self, state, h_state):
        """
        Return best action
        ...
        """
        action_values, h_state_out = self.sess.run(self.q, self.hs, feed_dict={self.s: [state], self.hs: [h_state]})[0]
        return np.argmax(action_values), action_values, h_state_out
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        s_batch, slen_batch, a_batch, r_batch, done_mask_batch, sp_batch, splen_batch = replay_buffer.sample_batch(
