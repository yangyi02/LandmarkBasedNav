...
class DRQN_instructor(object):
    ...
    def evaluate2(self, model_a, env=None, num_episodes=None):
        """
        Evaluation with same procedure as the training
        """
        # log our activity only if default call
        ...
        for i in range(num_episodes):
            ...
            for j in range(50):
                ...
                if action==1:
                    ...
                    goal_location_batch.append(goal_location)
                state, reward_i, done = env.step(0)
                if done:
                    goal_state_batch.append(goal_state)
                    goal_location_batch.append(goal_location)
                    break

            num_ldms.append(len(goal_state_batch)-1)
            reward_a = 0
            for j in range(len(goal_state_batch)):
                h_state_a = (np.zeros([1,model_a.config.h_size]),np.zeros([1,model_a.config.h_size]))
                model_a.env.teacher.set_goal(goal_state_batch[j], goal_location_batch[j])
                reward_a += model_a.navi_goal(h_state_a, goal_state_batch[j])
            if model_a.env.teacher.goal_finish:
                reward_a += 1
            else:
                success_flag = 0
    ...
    def gen_sample_seq(self, nway, ndigits, nins):
        """
        generate sample input and output sequences for planner
        the sequences contain following phases:
        description phase: with landmark pairs as input, no output
        query phase: with src and target pair as input, no output
        planning phase: no input or output, just for computation
        answering phase: input contains last state, output following instructor's discretization
        """
        self.env.reset()
        width, height = self.env.state.xmap.dim['width'], self.env.state.xmap.dim['height']
        num_classes = len(self.env.state.xmap.item_class_id)
        cur_perm = np.random.permutation(num_classes-1) # np.arange(num_classes-1) #
        cur_bgn = np.random.randint(num_classes-1)
        full_onehot_state = np.zeros((height+2, width+2, num_classes+2), dtype=bool)
        full_onehot_state[:,:,-1] = True
        full_onehot_state[1:-1, 1:-1, :] = self.env.state.origin_onehot_state
        #### remove agent
        full_onehot_state[self.env.state.agent_location[1]+1,self.env.state.agent_location[0]+1,18] = False
        full_onehot_state[self.env.state.agent_location[1]+1,self.env.state.agent_location[0]+1,0] = True
        state_seq = list()
        for i in range(2):
            if i<1:
                src_loc = self.env.state.xmap.items[i%5].location
                tgt_loc = self.env.state.xmap.items[(i+1)%5].location
            else:
                idx = np.random.choice(3,2,replace=False)
                src_loc = self.env.state.xmap.items[idx[0]].location
                tgt_loc = self.env.state.xmap.items[idx[1]].location
            succeed_flag, path_loc, path_ori = self.env.teacher.compute_shortest_path_single_src(self.env.state, src_loc, tgt_loc)
            assert(succeed_flag==True)
            h_state = (np.zeros([1,self.config.h_size]),np.zeros([1,self.config.h_size]))
            action = 0
            slen = np.ones(1).astype('int32')
            state_seq.append(list())
            for j, loc in enumerate(path_loc):
                state_ini = full_onehot_state[loc[1]:loc[1]+3, loc[0]:loc[0]+3, :]
                state = np.rot90(state_ini, path_ori[j])
                inner_state = np.argmax(state, axis=2)
                ## block view
                state[2,inner_state[1,:]==17,:] = False
                state[2,inner_state[1,:]==17,num_classes] = True
                raw_goal_state = np.concatenate([np.rot90(state, 0), np.rot90(state, 1), 
                    np.rot90(state, 2), np.rot90(state, 3)], 0)
                action, action_q, h_state = self.get_action(raw_goal_state[None][None], h_state, slen, [action])
                state = (state_ini.sum(0).sum(0)[1:-2]>0).astype('int32')
                state = state[cur_perm]
                if j==0 or j==len(path_loc)-1:
                    action = 1
                if action==1:
                    if state.sum()==0:
                        continue
                    if not state_seq[i]:
                        state_seq[i].append(DRQN_instructor.encode_state(state, nway, ndigits, nins, cur_bgn))
                    else:
                        if not (state_seq[i][-1]==DRQN_instructor.encode_state(state, nway, ndigits, nins, cur_bgn)).sum()==nins*nway*ndigits:
                            state_seq[i].append(DRQN_instructor.encode_state(state, nway, ndigits, nins, cur_bgn))
        encoding = list()
        encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[1,0]])[None])
        for i in range(len(state_seq)-1):
            for j in range(len(state_seq[i])-1):
                encoding.append(np.concatenate([state_seq[i][j],state_seq[i][j+1],[0,0]])[None])
                encoding.append(np.concatenate([state_seq[i][j+1],state_seq[i][j],[0,0]])[None])
        encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[1,0]])[None])
        encoding.append(np.concatenate([state_seq[-1][0],state_seq[-1][-1],[0,0]])[None])
        encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[1,0]])[None])
        for i in range(1):
            encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[0,0]])[None])
        encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[1,0]])[None])
        for i in range(len(state_seq[-1])-1):
            #encoding.append(np.concatenate([np.zeros(nins*nway*ndigits*2),[0,1]])[None])
            encoding.append(np.concatenate([state_seq[-1][i],np.zeros(nins*nway*ndigits),[0,1]])[None])
        encoding = np.concatenate(encoding, 0)

        target = np.zeros([encoding.shape[0], encoding.shape[1]-2])
        offset = encoding.shape[0]-(len(state_seq[-1])-1)
        for j in range(len(state_seq[-1])-1):
            target[offset+j] = np.concatenate([state_seq[-1][j],state_seq[-1][j+1]])

        pred_flag = np.zeros(encoding.shape[0])
        pred_flag[offset:] = 1
        return encoding, target, pred_flag

    @staticmethod
    def encode_state(state, nway, ndigits, nins, cur_bgn=0):
        encoding = np.zeros([nins, ndigits*nway])
        ins = np.where(state)[0]+1
        while len(ins)<nins:
            ins = np.append(ins, cur_bgn)
        for idx, x in enumerate(ins):
            for i in range(ndigits):
                encoding[idx, x%nway+i*nway] = 1
                x = np.floor(x/nway).astype('int32')
        encoding = encoding.reshape(-1)
        return encoding

    @staticmethod
    def decode_state(encoding, nway, ndigits, nins):
        state = np.argmax(np.reshape(encoding,[nins,ndigits,nway]),2)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        print(encoding[1])
        while 1:
            print(DRQN_instructor.decode_state(encoding[j][:nins*nway*ndigits], nway, ndigits, nins),
                DRQN_instructor.decode_state(encoding[j][nins*nway*ndigits:-2], nway, ndigits, nins))
            j += 1
            nldms[-1] += 1
            if encoding[j][-2]==1:
                j += 1
        ...
        while j<target.shape[0]:
            print(DRQN_instructor.decode_state(target[j][:nins*nway*ndigits], nway, ndigits, nins),
                DRQN_instructor.decode_state(target[j][nins*nway*ndigits:], nway, ndigits, nins))
            j += 1
